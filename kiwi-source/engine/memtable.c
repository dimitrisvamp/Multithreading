...
static int _memtable_edit(MemTable* self, const Variant* key, const Variant* value, OPT opt)
{
    ...
    // by the user.
    //
    pthread_mutex_init(&self->mem_lock, NULL);
    pthread_mutex_lock(&self->mem_lock);
    ...
    pthread_mutex_unlock(&self->mem_lock);
...
{
    pthread_mutex_init(&list->skiplist_lock, NULL);
    pthread_mutex_lock(&list->skiplist_lock);
    ...
    return 1;
    pthread_mutex_unlock(&list->skiplist_lock);
}
